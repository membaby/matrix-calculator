...
class Calculator(QWidget):
   def __init__(self, parent=None):
      ...
      self.combo_rootfinder_method.addItems(['Bisection', 'False Position', 'Fixed Point', 'Newton Raphson', 'Secant Method'])
   ...
   def solve(self, rootfinder=False):
      # try:
         if not rootfinder:
            ...
            task = {
               'precision': int(self.txt_rootfinder_precision.text()) if self.txt_rootfinder_precision.text() else self.DEFAULT_PRECISION,
               'tolerance': float(self.txt_rootfinder_EPS.text()) if self.txt_rootfinder_EPS.text() else self.DEFAULT_TOLERANCE,
               'number_of_iterations': int(self.txt_rootfinder_MaxIterations.text()) if self.txt_rootfinder_MaxIterations.text() else self.DEFAULT_ITERATIONS,
               'upper_boundary': float(self.txt_rootfinder_UpperBoundary.text()) if self.txt_rootfinder_UpperBoundary.text() else self.DEFAULT_UpperBoundary,
               'lower_boundary': float(self.txt_rootfinder_LowerBoundary.text()) if self.txt_rootfinder_LowerBoundary.text() else self.DEFAULT_LowerBoundary,
               'initial_guess': float(self.txt_rootfinder_InitialGuess.text()) if self.txt_rootfinder_InitialGuess.text() else self.DEFAULT_InitialGuess,
               'initial_guess_2': float(self.txt_rootfinder_InitialGuess2.text()) if self.txt_rootfinder_InitialGuess2.text() else self.DEFAULT_InitialGuess,
            }

            
            if method == 'Bisection':
               solver = Bisection()
               solution, steps, estimate_roots = solver.bisection(equation, task['lower_boundary'], task['upper_boundary'], task['tolerance'], task['precision'], task['number_of_iterations'])
               self.plot(method, equation, task, estimate_roots)

            elif method == 'False Position':
               solver = RegulaFalsi()
               solution, steps, estimate_roots = solver.regula_falsi(equation, task['lower_boundary'], task['upper_boundary'], task['tolerance'], task['precision'], task['number_of_iterations'])
               self.plot(method, equation, task, estimate_roots)

            elif method == 'Newton Raphson':
               solver = NewtonRaphson()
               solution, steps = solver.getSolution(equation, task['initial_guess'], task['tolerance'], task['number_of_iterations'], task['precision'])
               solver.plot()
               
            elif method == 'Fixed Point':
               solver = FixedPoint()
               solution, steps, values = solver.FixedPoint(equation, task['tolerance'], task['precision'], task['initial_guess'], task['number_of_iterations'])
               self.plot(method, equation, task, values)
            
            elif method == 'Secant Method':
               solver = Secant()
               solution, steps = solver.SecantMethod(equation, task['initial_guess'], task['initial_guess_2'], task['tolerance'], task['number_of_iterations'], task['precision'])
               solver.plot()
   ...
   def plot(self, method, equation, task, estimate_roots):
      ...
      if method in ['Bisection', 'False Position']:
         ...
         for x, y in estimate_roots:
            plt.plot([x, x], [x, y], 'g')
            plt.plot([x, y], [y, y], 'g')
         plt.xlim(min([x[0] for x in estimate_roots]+[task['initial_guess']])-2, max([x[0] for x in estimate_roots]+[task['initial_guess']])+2)
