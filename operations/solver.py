...
class Solver:
    @staticmethod
    def get_solution(parent, method, matrix, task):
        ...
        if method == 'Gauss Elimination':
            ...
            for i in range(len(matrix)):
                if matrix[i][i] == 0:
                    ...
                    parent.displaySolution('Gauss Seidel',
                                           ['There is no solution.', 'Diagonal element cannot be zero!'])
                    parent.displaySteps('Gauss Seidel', ['There is no solution.', 'Diagonal element cannot be zero!'])
                    return None, None
            solver = GaussSeidel()
            solution, steps = solver.getSolution(matrix, task['initial_guess'], task['tolerance'],
            ...
            for i in range(len(matrix)):
                if matrix[i][i] == 0:
                    parent.display('[ERROR] Diagonal element cannot be zero!')
                    parent.displaySolution('Jacobi Iteration',
                                           ['There is no solution.', 'Diagonal element cannot be zero!'])
                    parent.displaySteps('Jacobi Iteration',
                                        ['There is no solution.', 'Diagonal element cannot be zero!'])
                    return None, None
